class RegularGrid implements ColisionDetection {
  ...
  void hasMoved(Element currentElement) {
    ...
    //CALCULAR CORRECTAMENTE LA INTERSECCION PARA NO ELIMINAR DE MAS. Se supone q lo hace mas eficiente..se supone...(parece q es menos costoso eliminar que verificar la interseccion)
    /*if (dEndX > startX && dStartX <startX){
      int tmp = startX;
      startX = dEndX;
      dEndX = tmp-1;
   }else{
     if (dStartX < endX && dEndX >endX){
       int tmp = endX;
       endX = dStartX; 
       dStartX = tmp;
     }
   }
    if (dEndY > startY && dStartY <startY){
      int tmp = startY;
      startY = dEndY;
      dEndY = tmp-1;
   }else{
     if (dStartY < endY && dEndY >endY){
       int tmp = endY;
       endY = dStartY; 
       dStartY = tmp;
     }
  ...
  public void renderGrid() {
    fill(50, 100, 50);    
    for (int x=0;x<totalWidth;x++)  for (int y=0;y<totalHeight;y++)  if (grilla[x][y].values().size()>0) rect(x*maxSize, y*maxSize, maxSize, maxSize);
  }
}
