...
public class CellNoise {
	...
	*/
	public final int EUCLIDEAN = 1;
	public final int CITYBLOCK = 2;
	public final int MANHATTAN = 3;
	public final int QUADRATIC = 4;
	
	
	/*
	* Constructor.
	*/
	public CellNoise(PApplet parent) {
	...
	*/	
	private void noise3D(CellDataStruct cd) {
		...
		*/
		if ( x2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] - 1, int_at[1], int_at[2], new_at, cd);
		if ( y2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0], int_at[1] - 1, int_at[2], new_at, cd);
		if ( z2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0], int_at[1], int_at[2] - 1, new_at, cd);
		if (mx2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] + 1, int_at[1], int_at[2], new_at, cd);
		if (my2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0], int_at[1] + 1, int_at[2], new_at, cd);
		if (mz2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0], int_at[1], int_at[2] + 1, new_at, cd);
		
		/*
		* The 12 edge cubes. These are next closest.
		*/
		if ( x2 +  y2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] - 1, int_at[1] - 1, int_at[2], new_at, cd);
		if ( x2 +  z2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] - 1, int_at[1], int_at[2] - 1, new_at, cd);
		if ( y2 +  z2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0], int_at[1] - 1, int_at[2] - 1, new_at, cd);
		if (mx2 + my2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] + 1, int_at[1] + 1, int_at[2], new_at, cd);
		if (mx2 + mz2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] + 1, int_at[1], int_at[2] + 1, new_at, cd);
		if (my2 + mz2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0], int_at[1] + 1, int_at[2] + 1, new_at, cd);
		if ( x2 + my2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] - 1, int_at[1] + 1, int_at[2], new_at, cd);
		if ( x2 + mz2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] - 1, int_at[1], int_at[2] + 1, new_at, cd);
		if ( y2 + mz2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0], int_at[1] - 1, int_at[2] + 1, new_at, cd);
		if (mx2 +  y2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] + 1, int_at[1] - 1, int_at[2], new_at, cd);
		if (mx2 +  z2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] + 1, int_at[1], int_at[2] - 1, new_at, cd);
		if (my2 +  z2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0], int_at[1] + 1, int_at[2] - 1, new_at, cd);
		
		/*
		* The 8 corner cubes. 
		*/
		if ( x2 +  y2 +  z2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] - 1, int_at[1] - 1, int_at[2] - 1, new_at, cd);
		if ( x2 +  y2 + mz2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] - 1, int_at[1] - 1, int_at[2] + 1, new_at, cd);
		if ( x2 + my2 +  z2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] - 1, int_at[1] + 1, int_at[2] - 1, new_at, cd);
		if ( x2 + my2 + mz2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] - 1, int_at[1] + 1, int_at[2] + 1, new_at, cd);
		if (mx2 +  y2 +  z2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] + 1, int_at[1] - 1, int_at[2] - 1, new_at, cd);
		if (mx2 +  y2 + mz2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] + 1, int_at[1] - 1, int_at[2] + 1, new_at, cd);
		if (mx2 + my2 +  z2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] + 1, int_at[1] + 1, int_at[2] - 1, new_at, cd);
		if (mx2 + my2 + mz2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] + 1, int_at[1] + 1, int_at[2] + 1, new_at, cd);
	...
	*/	
	private void noise2D(CellDataStruct cd) {
		...
		* and most likely to have a close feature point.
		*/
		if ( x2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] - 1, int_at[1], new_at, cd);
		if ( y2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0], int_at[1] - 1, new_at, cd);
		if (mx2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] + 1, int_at[1], new_at, cd);
		if (my2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0], int_at[1] + 1, new_at, cd);
		
		/*
		* The 4 edge squares. These are next closest.
		*/
		if ( x2 +  y2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] - 1, int_at[1] - 1, new_at, cd);
		if (mx2 + my2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] + 1, int_at[1] + 1, new_at, cd);
		if ( x2 + my2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] - 1, int_at[1] + 1, new_at, cd);
		if (mx2 +  y2 < cd.F[cd.max_order - 1]) AddSamples(int_at[0] + 1, int_at[1] - 1, new_at, cd);
